...
grid_search = GridSearchCV(DecisionTreeClassifier(random_state=42), params_for_grid, cv=5, n_jobs=-1)

# Refitting the model using the updated hypermarameters
grid_cv = grid_search.fit(x_train, y_train)
best_params = grid_search.best_params_
new_model = DecisionTreeClassifier(**best_params, random_state=42)
...
plot_tree(new_model, filled=True, rounded=True)
